...
- 按 parallel-development 的 Merge orchestration 执行:
  1. Preflight:\`git switch dev && git pull --ff-only origin dev\`;\`git status --short\` 主树干净。
     ...
     其余未预期改动请停下报告。
  2. 按 manifest 顺序逐个 \`git merge --no-ff <branch>\` → 跑门禁(\`uv run mkdocs build -q\`;\`python3 scripts/check-characters.py\`;\`bash scripts/check-upstream-remnants.sh\`;\`git diff --check\`)→ 任一红则停。
...
- Env-only 步骤(workflow.json 的 env_only)不做。
...
claude -p \
  ...
  --name "boss-merger" \
  --output-format text \
  --allowedTools "Read, Grep, Glob, Edit, Write, Bash(cd*), Bash(git switch dev), Bash(git pull --ff-only origin dev), Bash(git status*), Bash(git log*), Bash(git branch --merged*), Bash(git worktree list), Bash(git worktree remove*), Bash(git branch -d*), Bash(git merge --no-ff*), Bash(git push origin dev), Bash(git diff*), Bash(git add*), Bash(git commit*), Bash(git checkout --*), Bash(uv run mkdocs*), Bash(python3 scripts/check-characters.py*), Bash(bash scripts/check-upstream-remnants.sh*), Bash(cat*), Bash(grep*), Bash(ls*), Bash(pwd)" \
  --disallowedTools "Bash(git push origin main), Bash(git push --force*), Bash(git reset --hard*), Bash(git rebase*), Bash(git worktree add*), Bash(git checkout dev), Bash(git checkout main), Bash(git checkout master), Bash(uv sync*), Bash(uv add*), Bash(uv remove*), Bash(yarn install*), Bash(yarn add*), Bash(npm install*), Bash(npm ci*), Bash(npm audit*), Bash(npx*), Bash(export*), Bash(chmod*), Bash(rm -rf*), Bash(sudo*)" \
  --add-dir "$BATCH" \
  < "$INST" \
  > "$LOG" 2>&1
