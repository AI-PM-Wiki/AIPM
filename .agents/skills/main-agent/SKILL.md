...
### 1. 理解目标
- 读 `AGENTS.md`、`.Codex/workflow.json`(批次目录/门禁以它为准);必要时并行派 Explore agent 摸清结构。
...
> 独立开发会话。先读 `AGENTS.md`、`.Codex/workflow.json`,再开工。
...
> `git submodule update --init --recursive`(详见 .Codex/workflow-notes.md)。
