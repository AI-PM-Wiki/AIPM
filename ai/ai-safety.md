...
高风险义务按阶段适用，不是「2026 年起一刀切」：禁令与 AI 素养义务更早生效，通用 AI（GPAI）与多数高风险系统的节点不同，部分随被监管产品上市的义务更晚。出海欧盟的产品从设计阶段按高风险条款自查，具体日期以欧盟官方文本为准。
...
-   **站内体系**：[提示词安全](prompt-security.md)（注入/越狱/红队/OWASP）、[内容安全与护栏](../practice/content-safety.md)（检测与护栏运营）、[出海与合规](../practice/compliance.md)（法规框架与备案）、[数据隐私与用户控制](../practice/data-privacy.md)（隐私机制）、[评估与评测](evaluation.md)（评测集与红队方法论）、[可解释性与安全前沿](interpretability-safety-frontier.md)（内部表征与监控的论文导读）。
