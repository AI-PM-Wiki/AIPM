...
-   **Whisper 系**：OpenAI 开源，多语言、多任务（转写、时间戳；`translate` 任务为语音译成英文，不是任意语对），离线批量质量高，社区生态成熟（[Whisper](https://github.com/openai/whisper)）；适合重质量、可离线、可私有化的场景。
...
-   **更多场景从级联迁到端到端**：旗舰 API 已提供端到端语音，大众产品仍大量用级联；迁移看延迟、情感保留与成本，而不是一次换成标配。
...
4.  AIGC 面试书多模态系列笔记（`sources/mm/`）：仅供维护者交叉验证，读者以 Whisper 仓库与各厂商多模态文档为准。
