...
扩展定律的工程用法：先用小规模实验拟合损失曲线，外推估算大训练任务的收益与成本；它也用来识别加数据没用了（边际收益递减）和模型欠训练（参数与数据不匹配）：但记住它预测的是语言建模损失，**不直接等于下游能力、事实性与产品体验**。与 Bitter Lesson、Densing Law、推理时扩展对照见 [AI 定理与经验定律](theorems.md)。
...
```mermaid
flowchart LR
    goal["明确目标能力"] --> schema["设计模板与数据 schema"]
    schema --> samples["收集真实任务样本"]
    samples --> label["标注 / 模型辅助生成"]
    label --> review["质量审核与事实校验"]
    review --> format["去重清洗 / 安全合规 / 格式校验"]
    format --> mask["设置 loss mask"]
    mask --> split["训练集 / 验证集划分"]
    split --> pilot["小规模试训"]
    pilot --> evaluate["评估后扩大"]
```

核心关系：SFT 数据先围绕目标能力组织，再经过质量、合规、格式和验证闸口，最后以小规模试训结果决定是否扩大。
...
```mermaid
flowchart LR
    corpus["预训练语料"] --> base["基座模型"]
    instructions["指令数据"] --> sft["SFT：指令微调"]
    base --> sft
    sft --> policy["策略模型"]
    preference["人类偏好排序"] --> reward["奖励模型 RM"]
    policy --> ppo["PPO + KL 约束"]
    reward --> ppo
    ppo --> aligned["对齐后的模型"]
```
