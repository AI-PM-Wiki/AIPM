...
```mermaid
flowchart TD
    observation[观察到价格与选择] --> budget[计算预算与可购买商品束]
    budget --> compare[比较已选束与未选束]
    compare --> contradiction{互相显示偏好？}
    contradiction -->|否| consistent[通过 WARP 检验]
    contradiction -->|是| violation[违反 WARP]
```

显示偏好检验用可观察的价格、支出和选择判断行为是否前后一致，而不直接假定消费者说出的偏好。
