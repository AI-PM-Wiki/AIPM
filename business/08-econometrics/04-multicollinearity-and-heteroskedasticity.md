...
```mermaid
flowchart LR
    regression[多元回归结果] --> corr[相关矩阵]
    regression --> vif[VIF 检验]
    regression --> joint[整体 F 显著但 t 不显著]
    corr --> diagnose[判断共线性范围]
    vif --> diagnose
    joint --> diagnose
    diagnose --> treatment[删变量、变换、增样本或岭回归]
```

多重共线性主要放大方差、削弱单个系数的识别，诊断后应结合经济含义选择处理方式。
