...
flowchart TD
    trilemma[开放经济政策选择]
    trilemma --> fixed[汇率稳定]
    trilemma --> independent[货币政策独立]
    trilemma --> flow[资本自由流动]
    fixed --> pair1[与独立货币政策二选一]
    independent --> pair1
    fixed --> pair2[与资本自由流动二选一]
    flow --> pair2
    independent --> pair3[与资本自由流动组合]
    flow --> pair3
```

三元悖论要求政策当局在汇率稳定、货币独立和资本开放之间取舍，不能同时获得三者。
