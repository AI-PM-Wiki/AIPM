...
```mermaid
flowchart LR
    disclosure[发行人信息披露] --> exchange[交易所审核]
    exchange --> registration[监管部门注册]
    registration --> issuance[发行证券]
    issuance --> listing[上市交易]
    listing --> supervision[持续信息披露与监管]
```

注册制把发行审核、注册、上市和持续监管连接起来，但发行人及中介机构仍需对信息披露质量承担责任。
