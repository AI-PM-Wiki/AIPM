...
```mermaid
flowchart LR
    macro[经济增长方式] --> industry[行业前景判断]
    cycle[产业周期] --> industry
    advantage[竞争优势] --> industry
    supply[供求态势] --> industry
    industry --> company[公司经营分析]
    company --> finance[财务分析]
    finance --> selection[投资标的选择]
```

投资标的选择沿着宏观、行业、公司和财务分析逐层收敛，最终把行业机会落实为具体公司判断。
