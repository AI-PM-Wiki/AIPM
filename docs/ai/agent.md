---
description: Agent 基本机制、感知-规划-行动-反思循环、工作流与产品设计要点
---
...
这九层里，最容易被产品团队砍掉的是评估与观测层：没有 trace 和评测，Agent 的失败无法复现，信任也无从建立。九层各有专页展开：规划与编排见 [Agent 架构与多智能体](agent-architecture.md)，工具与协议见 [工具调用与 MCP](agent-tools.md)，评测见 [评估与评测](evaluation.md)。
...
预算不是拍脑袋，而是可以提前算的。以自动生成行业调研报告为例：目标 1 万字报告，估算单任务平均 8 轮循环、每轮 3000 输入 token + 1500 输出 token，模型输出价 $10/百万 token：
