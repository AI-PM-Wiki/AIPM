---
description: AI 产品评估与评测：三层评估体系、评测集与线上指标、人工评测与红队测试
---
...
-   评估分三层：能力（模型本身）、任务（某个任务做得好不好）、产品（业务指标好不好）。三层混淆是 AI 评估最常见的错误。
...
-   **评测集污染**：测试用例一旦进入提示词或训练数据，该用例就失效。评测集要与生产链路隔离，定期轮换新增。
