---
description: 开源与闭源模型生态的格局、许可证实务、选型决策框架与迁移风险（2026-08 快照）
---
...
| [模型能力与选型](capabilities.md) | 单模型能力与定价快照、effort 档位、多模型路由 |
...
-   **2025 年**:推理能力开源化。DeepSeek-R1 用纯强化学习训出推理能力，并把思考过程蒸馏给 1.5B-70B 小模型，32B 蒸馏版在数学基准上超过 o1-mini（见 [模型训练与对齐](llm-training.md) 的蒸馏一节）
...
三条路不是互斥的终态，而是随业务阶段演进的路径:起步用闭源 API 快速验证，调用量大了把常规量迁到本地开源，最难任务保留 API 旗舰兜底（路由做法见 [模型能力与选型](capabilities.md) 与 [LLM API 与供应商](../tools/llm-api.md)）。
...
3.  **跑候选**:同一评测集跑开/闭/混合候选，记录正确率与错误类型分布（方法论见 [评估与评测](evaluation.md)）
...
7. [DeepSeek-R1](https://github.com/deepseek-ai/DeepSeek-R1):R1 模型卡、蒸馏与 MIT 许可
...
13. 站内关联:[模型能力与选型](capabilities.md)、[大模型基础](llm-basics.md)、[模型推理与部署](llm-inference.md)、[模型训练与对齐](llm-training.md)、[LLM API 与供应商](../tools/llm-api.md)、[LLM 成本测算](../tools/llm-cost.md)
