---
description: 提示词安全：注入与越狱、防护体系、红队、OWASP LLM Top 10 与合规，给出可落地的攻击面清单和上线前回归用例。防护要落在输入、工具和输出三层，而不是单条规则。
---
...
共性原则：**先只读后写入、高危动作必审批、所有副作用可追踪、测试集必须包含攻击与异常场景**（详见 [Agent 与工作流](agent.md) 与 [工具调用与 MCP](agent-tools.md) 的「权限隔离」）。
...
OWASP（开放全球应用安全项目）维护的 **Top 10 for LLM Applications** 是 LLM 应用安全的事实标准清单。下表按 **2026** 年版排序与名称转述（[OWASP GenAI LLM Top 10 2026](https://genai.owasp.org/resource/owasp-genai-llm-top-10-2026/)）：

| # | 2026 风险 | 一句话说明 |
| --- | --- | --- |
| LLM01 | 提示词注入 | 恶意指令混入用户输入、检索内容、工具结果或记忆，操纵模型行为（本文核心主题） |
| LLM02 | 敏感信息泄露 | 模型在回答、思考链或工具调用中泄露个人数据、密钥、内部信息 |
| LLM03 | 过度代理 | 功能、权限或自主性过大，被攻破后能在对话窗外造成实质伤害 |
| LLM04 | 供应链 | 第三方模型、插件、工具包或模型产物被篡改、带毒 |
| LLM05 | 数据与模型投毒 | 攻击者污染训练、微调、嵌入数据或权重 |
| LLM06 | 无限消耗 | 长上下文、递归调用、工具滥用导致资源与成本失控 |
| LLM07 | 错误信息 | 流利但错误的输出驱动错误决策或错误工具调用 |
| LLM08 | 隐藏上下文暴露 | 系统提示词与隐藏上下文被诱导输出（2025 年称系统提示词泄露） |
| LLM09 | 向量与嵌入弱点 | 检索投毒、嵌入反演、跨上下文泄漏 |
| LLM10 | 不当输出处理 | 模型输出未经校验就进入 SQL、代码、HTML 或下游系统 |

相对 2025 年版：过度代理从第 6 升到第 3；无限消耗从第 10 升到第 6；不当输出处理从第 5 降到第 10；系统提示词泄露更名为隐藏上下文暴露。条目以官网为准。

| 风险 | 常踩环节 | 常规应对 |
| --- | --- | --- |
| 提示词注入 | 用户输入、检索内容、工具回传 | 指令数据分层、权限、沙箱 |
| 敏感信息泄露 | 输出、思考链、日志、评测数据 | DLP、脱敏、访问控制 |
| 过度代理 | Agent 权限与不可逆操作 | 最小权限、人工确认、分阶段开放 |
| 供应链 | 第三方模型 / 插件引入 | 供应商评估、版本锁定、权重校验 |
| 数据与模型投毒 | 知识库上传、微调数据 | 内容审核、来源标注、隔离训练数据 |
| 无限消耗 | 长上下文、递归工具调用 | 预算上限、最大步数、成本监控 |
| 错误信息 | 事实类输出、工具参数 | 引用约束、拒答策略、人工抽检 |
| 隐藏上下文暴露 | 对话、共享链接、截图 | 提示词资产保护、泄露监测 |
| 向量与嵌入弱点 | RAG 检索链路 | 检索内容隔离、向量库访问控制 |
| 不当输出处理 | 模型输出拼接 SQL / HTML / 代码处 | 输出转义、白名单校验、不直接执行 |

与日常工作关系最密切的几条：**提示词注入**（所有输入环节）、**过度代理**（Agent 权限，见 [工具调用与 MCP](agent-tools.md)）、**隐藏上下文暴露**（提示词资产保护）、**不当输出处理**（模型输出进业务流程前的校验）。不当输出处理尤其隐蔽：模型输出如果被直接拼进 SQL、代码或 HTML，提示词攻击就升级成传统注入，这是产品架构层的责任。
...
> 以下来源访问验证日期 2026-09-04，结论以官方页面为准；本文为原创转述，未大段照抄原文。

1.  [Simon Willison: Prompt injection 系列文章](https://simonwillison.net/tags/prompt-injection/)：直接/间接注入的案例库、概念辨析与防御思路追踪。
2.  [OWASP GenAI LLM Top 10 2026](https://genai.owasp.org/resource/owasp-genai-llm-top-10-2026/)：现行十大风险排序与名称；2025 年版仅作对照。
...
6.  [RAG 基础](rag.md)、[Agent 与工作流](agent.md)、[工具调用与 MCP](agent-tools.md)、[评估与评测](evaluation.md)：间接注入场景、权限治理与红队评测的站内延伸。
