---
description: RAG 基础：入库与在线流程、切块检索重排、失败模式、hit rate 与 recall@k，以及 RAG、微调和长上下文如何组合而不是互相替代，并统一 L0 起步基线与 L1 生产标配
---
...
4.  拿不定？建 30-50 条评测集，三种方案各跑一遍，用指标说话。进阶组件与 0–5 级演进见 [高级 RAG](rag-advanced.md)。
...
-   **起步基线（L0）**：BM25 + 向量 + RRF 融合 + 提示词约束，不含 rerank。先用它建评测集、出基线。
-   **生产标配（L1）**：在 L0 上加 rerank 精排。大多数产品到此为止。原理见 [检索技术](rag-retrieval.md)。
...
术语不要混：L0 是起步基线，L1 才是多数线上产品的生产标配。演进地图见 [高级 RAG](rag-advanced.md)。

## 产品设计要点

-   **知识治理在前**：文档质量决定问答质量：先治理知识（结构化、去重、版本化、明确哪些可被问哪些保密），再做检索优化；给烂数据打补丁式的检索优化毫无意义（详见 [知识库问答](../practice/kb-qa.md)）。
-   **知识更新节奏**：明确源文档变更 → 索引同步的触发方式（定时任务或事件驱动）与同步延迟 SLA；文档改版后旧块要删除或替换，避免幽灵引用；更新链路要可监控、可回溯。增量更新（按文档 id + 内容哈希去重，只刷变化的文档）是推荐模式，配套做法见 [RAG 产品化实战](../practice/rag-practice.md) 的「索引运营」。
...
评估 RAG 产品，核心看三类指标（组件级指标与 RAGAS 见 [高级 RAG](rag-advanced.md) 的评测体系）：

1.  **检索命中**：该检索到的内容检没检索到。口径上分开：**hit rate**（top-k 里有没有至少一个应命中块）与 **recall@k**（多个应命中块时逮住了几个），另加 MRR（应命中块排在第几位）。定义见 [检索技术](rag-retrieval.md) 的检索评估。需要人工标注问题 → 应命中的文档/块 id。
...
-   **hit rate**：评测集里标注了应命中块 id 的问题，检索 top-k 结果**至少包含一个**应命中块的比例：衡量有没有检到。
-   **recall@k**：top-k 里命中的应命中块占全部应命中块的比例：一题有多块应命中时，看逮住了几个。定义见 [检索技术](rag-retrieval.md)。
...
-   **用 embedding 还是 BM25？** 语义匹配 vs 精确匹配，单用都有盲区：L0 起步基线就是混合召回（BM25 + 向量 + RRF），不要二选一；多数产品再加 rerank 成为 L1 生产标配。
...
| 混合检索 | 向量 + 关键词多路检索后融合 |
| hit rate | 至少一个应命中块是否出现在 top-k |
| recall@k | top-k 命中的应命中块占全部应命中块的比例 |
| MRR | 应命中块排名的倒数平均 |
