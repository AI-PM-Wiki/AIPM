...
```mermaid
flowchart LR
    acquire[取得固定资产] --> construction[在建工程]
    construction --> ready[达到可使用状态]
    ready --> depreciation[计提折旧]
    depreciation --> upgrade[更新改造]
    depreciation --> dispose[处置清理]
    upgrade --> ready
```

固定资产成本从取得开始，经过建造、使用期折旧和后续改造，最终在处置时结转清理损益。
