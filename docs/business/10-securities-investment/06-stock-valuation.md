...
flowchart TD
    stock[股票估值]
    stock --> absolute[绝对估值：未来现金流]
    stock --> relative[相对估值：可比倍数]
    absolute --> ddm[股利贴现 DDM]
    absolute --> fcf[自由现金流折现]
    relative --> pe[PE、PB、PEG]
    relative --> ev[EV/EBITDA、PS]
    ddm --> range[合理价值区间]
    fcf --> range
    pe --> range
    ev --> range
...
股票估值先按现金流或可比对象选择方法，再用多种结果交叉验证合理价值区间。
