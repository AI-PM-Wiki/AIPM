...
???+ note "网页批注"
    ...
    悬浮窗上「写批注」右侧那颗星是**「问助手」**：点它会把刚划的这段原文送进 AI 助手面板，摆在输入框上方，接着就能就着这段话提问。批注卡片上的「问助手」同理，送进去的是那条批注的引文与正文。送进去的语境一直留在输入框上方，追问同一段话不必重新送一遍；不需要了点它右端的叉收掉，换到别的页面也会自动收掉（它说的是「你正在读的这一页」）。

    **「仅本机」的批注送不进对话**：它的承诺是只在那台设备上，而语境会随提问发到问答后端。想跟助手讨论它，先在卡片上把它上传为公开或私有。

    **正文里的图也可以这样送进对话**：鼠标移到一张图或一张 Mermaid 图上，左下角会出现同一颗星。Mermaid 图送的是它的源码，SVG 送的是图里写的文字，位图送的是**图像本身**。位图只取同源的、认识的四类图片格式，且不超过 512 KiB；取不到时退回作者的替代文本（没写替代文本就留一句说明是哪一张、为什么没有内容），不会让按钮点下去毫无动静。
