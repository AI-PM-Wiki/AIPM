...
???+ note "网页批注"
    页面批注和文字高亮由本站自建服务提供，入口在页面右上角（搜索框与 GitHub 图标右侧）；选中正文里的任意一段话，就会浮出颜色选择条。
    ...
    **批注分三种，存放位置和可见范围各不相同：**
    ...
    面板顶部还有一个「智能高亮」按钮：它由模型判断页面里哪些段落值得划线、该用哪种颜色。使用该功能时，**该页面的正文片段会发送到模型服务**（用于判分的 TypeSafe 与 Anthropic）；结果只作建议，逐条或整体采纳后才会写入批注。这个功能不需要登录，也可以完全不使用。
