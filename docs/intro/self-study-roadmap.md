...
| 阶段 | 重点 | 最小产出 |
| --- | --- | --- |
| 阶段零 产品基本功 | 问题定义、用户研究、需求、设计、PRD、项目与数据 | 一份问题定义、用户流程和需求方案 |
| 阶段一 AI 系统认知 | 模型边界、上下文、RAG、工具调用、Workflow、Agent 与架构 | 一张系统图和 10 条能力边界记录 |
...
1.  [大模型基础](../ai/llm-basics.md)：token、上下文、生成、幻觉和能力边界
2.  [模型能力与选型](../ai/capabilities.md)：任务能力、成本、延迟、路由和版本变化
3.  [RAG 基础](../ai/rag.md)：知识来源、检索、引用和数据更新
4.  [Agent 与工作流](../ai/agent.md)：工具调用、状态、记忆、停止条件和失败恢复
5.  [工具调用与 MCP](../ai/agent-tools.md)：工具接口、权限、协议与执行边界
6.  [工程与架构](../tech/index.md)：请求链路、服务、数据、部署和 AI-Native 研发
...
判断标准：能解释系统为什么出错，并能提出验证方案；只会描述模型“聪明”或“笨”还不算完成。
...
选择一个阶段零定义的任务，用最小成本回答几个问题：模型能否完成，检索能否找到资料，工具能否安全执行，输出格式是否稳定，延迟和成本是否可接受。
...
-   **行业与交付**：领域流程、私有化、客户交付、规则编码、隐私与监管
