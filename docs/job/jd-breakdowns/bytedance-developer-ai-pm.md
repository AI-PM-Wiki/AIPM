...
这是一份面向开发者工具的 AI 产品经理 JD。职位名称写的是「AI 产品经理」，但团队方向、产品名称和后三张分析截图中提出的问题共同指向一个更具体的工作对象：把大模型能力嵌入代码开发流程，做出开发者愿意持续使用的产品。
...
!!! info "素材说明"
    ...
    原始 JD 只转写第一张截图，原图不在正文中直接展示；后三张截图仅作为后文分析的素材。

    第一张截图底部的职位要求未完整展示，本文不补写缺失内容。

## 原始 JD（OCR 转写）

!!! info "转写说明"
    以下文本按截图中可见的视觉内容转录，仅合并因版面造成的换行，不对原句润色。

    文字无法可靠辨认处标记为 `[无法辨认]`，截图边缘或底部缺失处标记为 `[截图截断]`；不根据上下文推测。

```text
AI产品经理-开发者AI

上海 | 正式 | 产品-产品经理 | 2025校招春招补录 |
职位 ID：A04263A

职位描述

团队介绍：字节跳动开发者AI团队致力于探索AI技术在研发体系上应用和落地，覆盖场景包括AI for Code、AI在DevOps全流程中的落地，团队包括产品、工程、算法和研究人员多个角色，期待对大模型、研发工具体系有兴趣、对技术有追求的同学加入，共同成长！

1、建设字节跳动豆包Marscode等AI编程产品，包括用户调研、行业分析、产品设计、用户体验设计、用户运营等；
2、充分传递产品价值，理解客户需求，并落实有效需求到产品中，持续跟进产品的关键数据指标和用户反馈，持续迭代优化产品；
...
4、具备良好的逻辑思维，有创新意识和意愿，有良好的[截图截断]
```
...
以下内容是从上方 OCR 文本提取的结构化摘要，不是招聘方额外写出的原话。
...
第四条要求在截图底部未完整展示，上方 OCR 仅保留了可见前缀。
...
后三张分析截图中的分析把用户分为专业开发者、非专业用户和企业团队。这是值得验证的用户假设，不是 JD 已经确认的用户画像。
...
后三张分析截图把 Git 管理、人员协作等内容列为可能的产品流程。企业团队场景还需要加入：
...
分析截图作者也明确表示对企业团队部分了解有限。这个不确定性本身就是面试时应主动验证的地方。
...
后三张分析截图列出了若干产品，目的是比较不同形态服务的需求。下表只整理分析截图中的形态信息，不把它当作完整的竞品评测。

| 产品 | 分析截图中的形态描述 | 可以比较的问题 |
| --- | --- | --- |
