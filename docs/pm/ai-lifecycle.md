---
description: AI 产品开发生命周期（CC/CD 框架）：持续校准/持续开发替代一次规划，代理权与控制权衡
---
...
用户只看到当前版本，但底层是逐级爬上来的。每个版本停留多久，取决于拿到多少行为信号。目标是搞清系统在真实噪声下的表现。放行下一级前先回答两个问题：**错误是否被约束在可回滚范围内？有没有评测证据证明当前级达标？**（代理权与信任的展开见 [Agent 产品](../practice/agent-product.md)）。
