...
本页是产品方法论栏目的入口与导航中枢，面向有经验的产品经理和准备 AI PM 岗位的候选人——这里不讲"什么是产品经理"式的科普，只讲能直接用的判断框架与决策链。栏目里的七篇文章不是并列的专题清单，而是一条从机会到商业的**决策链**——每篇回答一个具体决策问题：做什么、为谁做、怎么用、怎么按时交付、怎么迭代校准、怎么让开发照做、怎么赚钱。
...
| 产品设计 | 用户怎么理解和使用？失败时怎么兜底？ | 包含失败状态的交互原型 | 体验五层、交互范式决策链、错误注入测试（[产品设计与原型](design.md)） |
...
| [产品设计与原型](design.md) | 需求定义之后、开发之前 | 用户怎么理解？失败时怎么兜底？ | 包含失败状态的交互原型 | 把 AI 包装成无所不能；只画成功路径；让用户猜系统在干嘛 | 纯后端 API 或内部工具无界面时，只需能力边界与输出格式设计 |
...
这些枢纽页与七篇正文的关系：正文给判断框架，枢纽页给落地工具——评测、成本、合规是 AI 产品经理相对传统 PM 的新增必修课，正文里引用它们的地方就是衔接点。
