...
-   **错误消息是可恢复性的第一道门**：`failed` 让模型无从修复，只能原地重试烧 token；「订单号不存在，请确认后再查」则让模型能换参数再来。错误返回至少包含错误类型、可行动提示、稳定结构三样
...
MCP（Model Context Protocol）是让工具可插拔的协议层：工具提供方变成标准 Server，任何遵循协议的 Host 都能连接（协议机制与 Host/Client/Server 架构见 [工具调用与 MCP](../ai/agent-tools.md)）。对产品经理，MCP 的真正含义不是技术协议，而是**工具生态成为可能**：
