...
```mermaid
flowchart LR
    question[用户问题] --> understand[查询理解：改写、拆解、澄清]
    understand --> sources[多源检索：网页、新闻、垂直源]
    sources --> rank[重排：相关性、时效、权威度]
    rank --> synth[综合生成：结构化答案与冲突呈现]
    synth --> cite[引用校验与编号标注]
    cite --> answer[带来源的答案]
    sources -.快照未命中.-> realtime[实时补抓]
    realtime --> rank
```

这条管线把 AI 搜索的责任链串起来：检索决定证据，重排决定来源质量，生成负责综合，引用校验决定答案能否被核对。
