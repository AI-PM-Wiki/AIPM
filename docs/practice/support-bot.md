...
**关键提醒：第一版往往不是「自动回复」，而是「路由」**。[PRD](../pm/prd.md) 的客服工单示例 v1 只做工单路由、不做自动回复——先让系统证明「判断准」，再让它「开口说」，最后才让它「自己办」。跳过 v1/v2 直接上 v3 是客服 AI 项目最常见的失败方式：错误不可控、用户信任崩塌、项目被叫停。
...
四层之间有依赖关系：检索层喂对话层（召回片段 → 生成回答），对话层喂动作层（意图 → 动作选择），人工层兜底所有层（低置信的检索、越界的意图、高风险的动作都汇到人工层）。排查问题按层定位：答非所问先查检索与对话，动作出错查权限与审计。与相邻形态的关系：客服机器人 = 知识库问答（检索 + 对话）+ 动作层（工单集成）+ 人工层（转人工）；相比 [Agent 产品](agent-product.md) 自主度受限，「权限最小化 + 人工兜底」是常态而非可选项。
...
```text
[入口] 鉴权 · 输入校验 · PII 脱敏 · 限流
   ▼
[检索+分类] 意图分类 + 知识库检索（RAG） ──低置信 / 敏感 / 用户要求──▶ 转人工
   ▼
[生成] 主模型：回答 / 路由建议 / 解决方案建议（记录模型 + prompt 版本与置信度）
   ▼
[输出护栏] 内容安全过滤 + 格式校验 ──拦截失败──▶ 降级话术 / 转人工
   ▼
[动作层] 需执行的动作：查单 / 改地址 / 路由工单（权限 + 审计）
   ▼
[人工层] 客服采纳 / 修改 / 拒绝，修改写回日志 → 评测数据回填
   ▼
[观测层] 全链路日志 · 成本看板 · 预算熔断 · feature flag
```
...
转人工不是失败，是**系统设计的一部分**——把「什么时候交给人类」写成显式规则，而不是等模型「自觉」：
...
转人工率的正确用法：**该转不转 = 客诉升级，不该转乱转 = 成本没省**。健康的产品不是「转人工率越低越好」，而是「该转的都转了、不该转的没转」——把「转人工」从产品设计里去掉，等于把安全阀焊死。
...
-   **出口藏太深**：转人工按钮藏在多级菜单里，用户找不到出口，只能愤怒刷屏——转人工入口在对话界面的常驻位置。
