---
---
description: AI 产品经理效率工具清单：对话、笔记、搜索、文档办公与会议纪要等单兵工具。
---
...
```mermaid
flowchart LR
    collect[搜索、会议与业务数据] --> notes[笔记或共享文档]
    notes --> generate[对话工具生成摘要、报告或任务]
    generate --> review[人工核对事实与口径]
    review --> action[进入任务、周报或团队协作]
```

效率工具的共同链路是收集、沉淀、生成、人工把关再行动；工具不替代事实核对和任务闭环。
...
三类工作流都用共享中转站承接工具输出，并保留人工确认作为对外发布前的最后一道闸门。
