---
description: 硬件相关岗位解读：硬件 PM、嵌入式、AIoT、机器人职责与软硬一体协作
---
...
| 嵌入式软件工程师 | 设备端软件：固件、RTOS、驱动、OTA | 硬件里的软件 |
| 硬件工程师（电子/结构） | 电路、结构、热设计、认证测试 | 硬件的本体 |
...
核心关系是：硬件以样机、试产和量产冻结点推进，变更代价随阶段上升，因此必须把测试和问题决策前置。
...
-   **「NPI / 量产经验」**：从做过 demo 到做过量产的分水岭。demo 谁都能做，量产才是真本事
-   **「软硬一体」**：指同时懂设备、App 与云端的岗位，随 AIoT 普及需求在涨，也是薪资高地
-   **警惕点**：名义「硬件产品经理」实为采购 / 跟单的 JD；智能硬件但研发全外包、无自有研发能力的岗位，成长空间有限
...
-   **联合定义用户旅程**：从开箱、配对、使用、OTA 升级到售后，软硬两端各自负责的环节要无缝衔接。体验断层（App 说有、设备没有）是这类产品最常见的失败原因
...
-   **端侧 vs 云端**：算力、功耗、内存约束决定模型放在哪里；麦克风阵列、摄像头选型直接决定算法上限。硬件 PM 与算法对齐规格，算法用实测数据反哺选型
...
-   **里程碑对齐**：样机、试产、量产三个节点，软件版本与硬件状态同步冻结；每个节点都做软硬联合验收
...
1.  **补硬件基础**：懂研发流程与成本结构（模具、物料、良率、认证），不一定要会画板子，但要听懂研发在说什么
...
-   [AI 产品经理](ai-pm.md)：软件侧 PM 的思考方式与协作习惯
