---
description: AI 产品开发生命周期：用持续开发与持续校准（CC/CD）替代一次规划，按代理权阶梯上线、评测、修复，让系统靠表现逐步扩权。
---
...
全文按「为什么不一样 → 循环总览 → CD 各阶段可操作清单 → CC 各阶段运营 → 成熟度模型 → 完整走查」组织，每个阶段都给清单、表格与模板，可直接抄进立项文档和 [PRD](prd.md)。
...
影响是什么？我们为**大概率行为**设计，而不是为可预期的用户流程设计；全程持续校准预期与现实。设计评审里那句"这个功能谁会这么用"在 AI 产品里必须换成"这个输入模型会怎么错"。
...
代理权与人工交接的实操细节见 [Agent 产品](../practice/agent-product.md)。
...
```mermaid
flowchart LR
    cd0[CD 0 问题与可行性] --> cd1[CD 1 划定能力范围]
    cd1 --> cd2[CD 2 搭建应用]
    cd2 --> cd3[CD 3 设计评测]
    cd3 --> deploy[部署：小范围灰度]
...
图示说明：首次进入或问题被推翻时从 CD 0 起；常规校准循环从 CD 1 再切一版代理权。CD 把能力做成可观测版本，CC 用真实反馈决定是否扩权。

???+ note "CC/CD 循环总览"
    **CD 0 问题与可行性** → CD 1 划定能力范围 → CD 2 搭建应用 → CD 3 设计评测 → **部署（过渡）** → CC 4 运行评测 → CC 5 分析错误模式 → CC 6 应用修复 → 回到 CD 1；问题定义被推翻则回到 CD 0。

    1.  **CD 0 问题与可行性**：任务能否拆开、容错率、数据、标签、基线与经济价值；用 spike 拿到可复现数字
    2.  **CD 1 划定能力范围**：按代理权层级切版本，整理参考数据
    3.  **CD 2 搭建应用**：不过度工程化，埋好日志，设计控制交接
    4.  **CD 3 设计评测**：为"非确定性的任务做得好不好"定标准
    5.  **部署**：不是终点，是进入校准的过渡，先部署到小范围人群
    6.  **CC 4 运行评测**：用真实交互数据检验效果
    7.  **CC 5 分析错误模式**：人工审查最弱环节，把错误模式文档化
    8.  **CC 6 应用修复**：数据驱动地修，改完重跑评测
...
循环节奏没有固定天数，由「拿到多少可信行为信号」决定：灰度样本不足就继续观察，指标稳定就升级。排期与跟踪见 [项目管理与迭代](project-management.md)。
...
| 检查项 | 要回答的问题 | 不过关的信号 |
| --- | --- | --- |
| 任务可分解性 | 任务能否切成可验证的子步骤？ | 只能整段评价感觉好不好，拆不出可断言步骤 |
...
| 经济价值 | 节省的人力、提升的转化是否覆盖模型成本？ | 收益与成本同数量级甚至更小（算账方法见 [LLM 成本测算](../tools/llm-cost.md)） |
...
spike 的产出是**一串可复现的数字**，不是"模型很厉害"的感慨：准确率、召回率、延迟、成本、失败率。这些数字直接成为 [PRD](prd.md) 能力边界与评测通过线的第一版依据。

### CD 1 划定能力范围 + 整理参考数据

#### 能力范围与代理权阶梯

版本按**代理权层级**切，而不是按功能集。对外发布可以用 v1 / v2 / v3 命名，对内必须能指出落在 L0–L4 的哪一级。把宏大终态拆成可评测、可迭代、可叠加的早期行为：
...
v1 / v2 / v3 是**产品版本切片**，不是另一套等级。本栏目代理权的权威口径是下面的 **L0–L4**。客服例子里，v1 路由通常落在 L0–L2（建议到确认后执行），v3 自动解决才接近 L3；不要把三个版本和五级阶梯一一对应。
...
| --- | --- | --- | --- | --- | --- |
| L0 只读/建议 | 读数据、出建议，不落库不改状态 | 无（冷启动即开放） | 输出标注仅供参考 | 用户忽略即可 | 数据被持续采用 |
...
用户只看到当前版本，但底层是逐级爬上来的。每个版本停留多久，取决于拿到多少行为信号。目标是搞清系统在真实噪声下的表现。放行下一级前先回答两个问题：**错误是否被约束在可回滚范围内？有没有评测证据证明当前级达标？** 界面怎么表达每一级，见 [产品设计与原型](design.md) 的代理权阶梯表。代理权与信任的展开见 [Agent 产品](../practice/agent-product.md)。
...
| 工具调用 | 工具清单最小化、入参出参 schema、权限边界 | 工具权限过宽，出 AI 把公司文件删了级事故 |
...
LLM-as-judge 大幅降低人工评测成本，但必须校准：先检查位置偏差、长度偏差、自我偏好三类系统偏差，再用一小批人工标注样本验证 judge 与人工的一致性达到阈值（如 80%）才可批量使用（详见 [评估与评测](../ai/evaluation.md)）。**Judge 是过滤器和初筛，不是最终裁判。**
...
| 单次解决成本 | 总 token 成本 ÷ 解决的请求数 | 线上全量；按月聚合 | 单位经济，与人工处理成本对比（见 [LLM 成本测算](../tools/llm-cost.md)） |
...
抽样信号示例：客服 v1 用"是否被人工改路由"作为路由准确率的代理指标；复杂系统看对话轮数、赞/踩等。跑评测要固定"评测集版本 + 模型版本 + prompt 版本 + 采样日期"四元组，否则分数之间不可比。
...
错误模式要有统一的分类学，否则复盘会变成各说各话。建议按错误发生的位置分类：
...
拿到错误样本后按数据 / 检索 / prompt / 模型 / 工具 / UI 六个方向做根因分解，每个方向对应不同的修复手段与回归方式：
...
修复优先级 = 影响面（出错频率 × 单次代价）× 修复成本倒数。不要按哪个好修排序，按哪个最痛排序。

效果归因的纪律：一次只改一个变量；改前改后在同一评测集版本上对照；线上指标要区分混杂因素（节假日、活动、流量变化）；改完必须回到 CC 4 重跑，不靠感觉变好了。
...
换模型在传统软件里约等于换数据库驱动，在 AI 产品里**等同于改产品范围**：能力边界、失败模式、成本结构、延迟、评测基线全部会变。必须按变更管理对待：

| 动作 | 要求 |
| --- | --- |
| 固定版本 | 生产环境锁定模型版本与 API 版本，不追最新；记录 prompt 版本 |
...
| 成本变化 | 单价、缓存折扣、批处理折扣都可能变，重新算单位经济（见 [LLM 成本测算](../tools/llm-cost.md)） |
| 服务条款变化 | 数据使用政策、合规承诺可能变化，法务过一遍（见 [出海与合规](../practice/compliance.md)） |

判断标准：**任何模型变更都要有评测集分数、在线指标、成本、失败模式迁移四张对照表，缺一张不许上线。**

### 安全、合规与组织流程

-   **敏感场景审批**：高风险功能（金融、医疗、未成年人、自动分发）上线前走审批链，审批依据是评测报告 + 责任表，不是感觉没问题
...
第 6~7 步的顺序：**先有数据证明"犯错可控"，才拿到下一级代理权。**
...
面试 AI 产品设计题时，用 CC/CD 框架回答"如何从 0 设计一个 AI 功能"：先按代理权拆 v1 → v2 → v3，再讲参考数据、评测指标、人工兜底。比直接堆功能清单高级得多。
...
两句金句：**"代理权要赚取，不要授予"**；**"Never lead with the tech"**。

该框架也能反哺日常判断：任何"直接上全自动"的方案，都值得先问一句"能不能先做成可覆写的建议"。面试追问"换模型怎么评估"或"评测失效怎么办"时，用本文的"模型与供应商变更"和"CC 阶段运营"两节回答，体现真正的实操深度。
...
???+ note "来源说明"
    ...
    -   **站内体系**：评测方法论见 [评估与评测](../ai/evaluation.md)，Agent 设计见 [Agent 产品](../practice/agent-product.md)，成本测算见 [LLM 成本测算](../tools/llm-cost.md)，合规红线见 [出海与合规](../practice/compliance.md)
