---
description: 面向产品经理的数据分析入门：用指标分层、漏斗、留存、同期群和看板把业务问题译成可行动的数，并覆盖 AI 产品的采纳漏斗与 badcase 回流。
---
...
本页是操作层入门：指标怎么定义、漏斗和留存怎么拆、看板怎么搭。实验设计与 A/B 统计见 [设计评审与可用性度量](design-evaluation.md)，北极星的商业含义与 LTV/CAC 见 [商业化与增长](monetization.md)，AI 输出质量的三层评估见 [评估与评测](../ai/evaluation.md)，埋点与数据回流见 [数据与标注](../tools/data.md)。漏斗留存先看本页，能不能支撑收钱再看商业化，不要两页各定一套北极星。
