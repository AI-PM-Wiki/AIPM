---
description: 设计评审与可用性度量：用专家评估、可用性测试、SUS / HEART 与 A/B 实验给设计假设找证据，并和用户研究、模型评测划清分工。
---

## 设计评审与可用性度量

设计好不好，不能靠"我觉得"。**设计的价值靠证据说话**：专家评审、可用性测试、量化指标与实验，是设计决策的四种证据来源。本页写**如何度量、如何做因果**；何时做研究、发现如何写进需求，见[用户研究](user-research.md)的可用性测试。AI 产品的设计评审检查表与「对话式 UI 的可用性测试要点」见[产品设计与原型总览](design.md)，AI 输出质量的评测见 [评估与评测](../ai/evaluation.md)。
...
```mermaid
flowchart LR
    expert[专家评审：发现疑似问题] --> users[可用性测试：确认真实卡点]
    users --> metrics[量化度量：判断严重程度]
    metrics --> experiment[A/B 实验：验证修复因果]
```

图示说明：评估方法沿成本和证据强度逐级加深，先用便宜方法缩小问题，再用实验确认改动是否真正有效。
...
```mermaid
flowchart TD
    goal[业务目标：提升 AI 助手的用户价值] --> north[北极星：每周完成任务的活跃用户数]
    north --> heart[HEART 分解]
    heart --> task[任务成功：完成率、任务时长、错误率]
    heart --> retention[留存：次日/7 日留存]
    heart --> engagement[参与度：人均会话数、周使用天数]
    north --> flows[下钻到具体流程]
    flows --> conversation[对话问答：首问成功率、追问率、转人工率]
    flows --> summary[文档总结：采纳率、编辑率、放弃率]
    north --> governance[每条指标：定义、口径、目标值、owner、采集方式]
```

图示说明：指标树从业务目标和北极星向下连接 HEART 维度与具体流程，每个指标都必须可解释、可行动且有负责人。
