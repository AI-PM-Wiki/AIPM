---
description: 交互设计基础：用心智模型、反馈、设计定律、导航、表单和可访问性设计用户与系统的对话，并为 AI 的不确定输出准备兜底。
---
...
交互设计的对象有四类：**流程**（任务怎么走完）、**状态**（系统处于什么阶段）、**反馈**（每一步的响应）、**行为**（系统主动做什么）。AI 产品尤其如此：模型调用、工具执行、自动建议都是「系统行为」，这些行为同样需要设计——何时触发、如何呈现、失败如何兜底（详见 [总览](design.md) 的「状态迁移」与 L0–L4 代理权阶梯）。
...
```mermaid
flowchart LR
    design[设计模型] --> image[系统表象]
    image --> mental[用户心智模型]
    mental --> goal[用户目标]
    goal --> action[用户操作]
    action --> feedback[系统反馈]
    feedback --> mental
    goal -. 执行鸿沟：不知道怎么做 .-> action
    feedback -. 评估鸿沟：看不懂结果 .-> mental
```

图示说明：系统表象把设计模型翻译成用户可行动的心智模型，操作线索与反馈分别填平执行鸿沟和评估鸿沟。
