---
description: AI 产品设计与原型总览：从问题框架到失败兜底的九步设计链路，覆盖体验五层、交互范式、原型策略和评审验收，并指向七个设计深度页。
---
...
客服工单助手的状态机片段（产品版本 v1 路由 / v2 建议 / v3 自动解决是 [CC/CD 生命周期](ai-lifecycle.md) L0–L4 上的切片，不是另一套等级）：
...
系统的代理权必须分级，每级对应不同的界面表达。**代理权要靠表现赚取，不要一次性授予**。等级定义以 [CC/CD 生命周期](ai-lifecycle.md) 的 L0–L4 为准；本表只补界面。审计是 L4 的界面要求，不是独立于自动执行的第六级。
...
-   [AI 产品开发生命周期（CC/CD）](ai-lifecycle.md)（L0–L4 代理权阶梯与控制交接）
