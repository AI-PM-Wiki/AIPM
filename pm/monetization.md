...
商业化回答「产品如何赚钱」，增长回答「如何让更多用户用起来」。在 AI 产品里两者深度耦合：推理成本随用量上升，增长会直接吃掉毛利，所以商业化要当成一个「从判断、设计到放大、守住」的完整系统。与 SaaS 相比，AI 产品的商业化有四大特殊性：
...
写作口径：**所有数字建议均为「演示假设」，不得与官方价格混淆**；价格类信息以官方页面为准（截至 2026-08-23 验证可达的官方定价页，引用日期标注在各案例小节）。
...
```mermaid
flowchart TD
    value[价值形态] --> recurring{持续使用?}
    recurring -- 是 --> predictable{用量可预测?}
    predictable -- 是 --> seat[席位订阅]
    predictable -- 否 --> hybrid[订阅 + 按量/用量包]
    recurring -- 否 --> usage[按量计费/用量包]
    seat --> payer[结合付款方设计套餐]
    hybrid --> payer
    usage --> payer
```

图示说明：商业模式先按价值是否持续、用量是否可预测分流，再把计费方式与实际付款方的购买逻辑对齐。
...
价值等价分析的目的是找到「用户的参照物」：用户拿你的价格和「他心中的替代方案价格」比，而不是和你的成本比。步骤：
...
-   **免费层**：承担「尝鲜 + 传播 + 收集数据」，必须低成本承载：限制高频动作（每日次数）、高级能力（长上下文、深度推理），而不是只限数量不限成本。免费层成本必须低于「免费用户带来的传播价值 + 未来转化期望」
...
```mermaid
flowchart LR
    revenue[收入] --> direct[推理与算力成本]
    direct --> margin[毛利率]
    guardrail[护栏、评测与人工兜底成本] --> margin
    margin --> decision[定价、路由与用量护栏]
    decision -. 用量与成本数据 .-> revenue
```

图示说明：AI 的 unit economics 把推理和护栏成本一起纳入毛利，再用毛利结果反向约束定价、模型路由与用量策略。
...
```mermaid
flowchart LR
    input[用户/需求输入] --> action[使用产品完成任务]
    action --> output[产生可用或可分享结果]
    output --> next[传播、协作或再次使用]
    next --> input
```

图示说明：增长循环的关键不是单次转化，而是产出能否带来下一次输入并在衰减可控时持续复利。
...
**免费层成本控制**：免费层的成本必须低于「免费用户的传播价值 + 转化期望」：用频率限制、模型路由（免费用户走小模型）、缓存压制成本。免费层设计的目标是「体验惊艳但不够用」，不是「体验完整」。
...
增长黑客的经典框架 AARRR（获取、激活、留存、变现、传播）在 AI 产品中依然适用（[爱范儿专访范冰](https://www.ifanr.com/621422)），但它回答的是「哪一环在漏」，不回答「这一环值不值得补」：AI 产品里每一环的量都可以花钱买出来，唯独毛利不行（见 [定价模型与 unit economics](#定价模型与-unit-economics) 与 [反模式](#反模式)）。
