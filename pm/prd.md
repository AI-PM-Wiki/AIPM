---
description: AI 产品 PRD 模板：用十节结构把能力边界、评测标准、失败路径和成本预算写成开发可执行、验收有据的文档，并给出工单助手示例与评审清单。
---
...
PRD（Product Requirements Document）是 PM 的核心交付物：把判断与设计落成开发照做的文档。在 AI 产品里，它尤其重要：需求分析解决「做什么」（见[需求分析](requirements.md)），产品设计解决「怎么用」（见[产品设计与原型](design.md)），PRD 解决的是**把判断写清楚，让开发照做、让验收有据**。

10 节模板、一份写得到位的示例节选、一张评审自查清单：照模板写，评审会少吵一半。写作前提是理解 AI 产品的运行节奏：上线只是 [CC/CD 生命周期](ai-lifecycle.md) 里校准循环的开始，PRD 要为每一轮校准留下可更新的接口。
...
**为什么不能「传统 PRD 加一段 AI 说明」了事**：传统 PRD 的粒度是「每条路径都写清」，AI 产品写不清所有路径（输入开放式），写了也测不完；传统 PRD 的验收是「断言等于预期」，AI 只能「评测好坏程度」。补一段说明不够：文档的骨架要从「功能规格」换成「能力契约」，这是结构差异，不是篇幅差异。

### PRD 在文档体系中的位置

PRD 是文档族的枢纽。每个 AI 功能从立项到上线，至少涉及这八类文档，各回答一个问题：
...
-   **例如**："路由准确率 ≥ 90%，对抗集 ≥ 80%，人工改路由率 ≤ 5/万单（0.05%）"
...
-   **例如**："先给 10% 工单开'路由建议'（不自动转派）；改路由率超 5/万单（0.05%）或延迟超 3 秒即回滚，责任人：客服主管"
...
只摘能力边界、评测标准、失败路径三节（v1 只做工单路由，不做自动回复）：
...
-   **线上指标**：人工改路由率（≤ 5/万单，即 0.05%）、平均处理时长、用户投诉率
...
???+ warning "PRD 不是一次写成的"
    ...
    真实用户行为与评测集会持续变化，PRD 要随评测与反馈迭代，这正是 [CC/CD 框架](ai-lifecycle.md) 的节奏。
...
**PRD 的终点是上线后评测达标。** 文档里每多一个可度量的数字，上线后就少一分开盲盒的风险。
