---
description: AI 产品项目管理与迭代：在不确定性下并行发现与交付，用两快一慢节奏、评测门禁、灰度和干系人 RACI 保证按时按质上线。
---

## 项目管理与迭代

项目管理保证产品**按时、按质**交付。AI 产品的项目节奏与确定性软件有明显差异：它管理的主要对象是**不确定性**：模型效果不确定、用户输入不确定、成本不确定。AI 项目管理是一套在不确定性下持续校准价值与交付节奏的系统，而不是更严格的排期。

### 先厘清：项目、项目管理与软件工程

**项目（Project）**：为创造独特产品、服务或成果而进行的**临时性**工作：有明确的起止时间、目标与资源约束（PMBOK 定义）。它是跨行业的通用容器：建筑、活动、科研、软件开发都用项目管理来组织。

**软件工程（Software Engineering）**：系统化开发与维护软件的**工程学科**：需求工程、架构设计、测试、质量保证与过程管理（瀑布/敏捷等）。软件工程是软件开发这门手艺，不是项目的一种。软件开发（工作）以项目形式组织，项目是工程实践的载体。
...
**软件开发项目 = 项目管理 × 软件工程 × 领域知识**。项目里的范围、时间、成本管理，软件工程不负责；软件工程里的架构、测试、质量实践，项目管理也覆盖不到。产品经理要懂的是两者的分工：项目管理管按时按质，软件工程管造得对不对；具体要懂到哪一层，见[能力模型](../intro/capability.md#②-ai-系统与工程理解)里的「② AI 系统与工程理解」。

## 项目类型与治理强度

不同项目类型的治理强度差异巨大：把 AI 项目一律按互联网迭代或一律按交付项目管理，都会出事。先分类再定治理：
...
治理强度怎么定：**风险越高、外部承诺越硬，文档与评审越重**。探索型项目用实验记录 + 周评审就够，硬套阶段门禁会扼杀探索；企业交付型没有门禁，会赔上合同与合规。
...
```mermaid
flowchart LR
    discover[发现轨道：问题与方案验证] -->|迭代开始：已验证想法进 backlog| deliver[交付轨道：构建可发布增量]
    deliver -->|迭代结束：真实数据回流| discover
```

图示说明：发现与交付并行推进，在迭代起点合流、在迭代终点用真实数据重新校准，而不是先发现后交付的一次性接力。
...
合流点：**每个迭代开始时**，把发现轨道上已验证的想法排进交付 backlog；迭代结束时，交付轨道带回真实数据，重新喂给发现轨道。合流失败的症状：发现轨道自嗨（做了一堆没进 backlog 的原型），或交付轨道自嗨（做的功能没经过任何验证）。

发现轨道的常用方法（按成本从低到高）：客户访谈与门童测试（验证问题真伪）→ 假门 / 落地页测试（验证需求强度）→ 绿野仙踪（人工冒充 AI 验证价值假设，详见 [产品设计与原型](design.md) 的原型策略）→ 可行性原型（小样本跑通 RAG / Agent 链路，验证 AI 的可行性风险）。规则：**发现轨道只产出已验证的想法 + 评测用例，不产出代码**。代码留给交付轨道。
...
规则：**流程是工具不是信仰**。AI 产品常见的选择是 Kanban 做发现、Scrum 做交付的混搭，比二选一更实用。
...
flowchart TD
    ...
    okr --> bet[赌注]
    bet --> milestone[里程碑]
    milestone --> commitment[高诚信承诺]
```

图示说明：路线图层级把长期方向逐层收敛为可衡量目标、资源赌注和经过发现验证后的交付承诺。
...
《启示录》对路线图的批判：**想法一旦写进名为 roadmap 的文档，全公司就会把它当成承诺**。此刻既不知道能赚多少钱，也不知道成本是多少，商业论证的输入是"无法知道的东西"。替代方案是上面这组层级：愿景负责激励，战略负责聚焦，OKR 负责度量，赌注负责投入，承诺只在发现完成后给出。
...
```mermaid
flowchart LR
    regression[周一：评测集回归] --> repair[周二~周三：数据驱动修复]
    repair --> rollout[周四：灰度发布与人工抽检]
    rollout --> retro[周五：回顾与扩充评测集]
    retro -. 下一周 .-> regression
```

图示说明：AI 迭代把评测放在周期开头和结尾，开发与发布围绕“修复—灰度—回流”循环，而不是只围绕写代码排期。

-   **验证快**：Prompt 调优、模型切换可以在小时级完成
-   **上线快**：Agent/API 集成比从零写功能快得多
-   **评估慢**：效果好坏难以自动化判定，回归测试成本高

评估慢决定了一切节奏设计：**快的是写代码，慢的是证明它好**。迭代节奏围绕评估来排，而不是围绕开发来排。典型的一周节奏：
...
1.  **定义成功指标**：每个迭代明确什么算好（准确率、任务完成率、用户留存）
2.  **建立评测集**：把典型用户输入沉淀为评测集，每次改模型/提示词都跑一遍
3.  **小步快跑**：Prompt 调整也要走改 → 评测 → 上线 → 观察闭环
...
| 切分方法 | 按代理权切（对齐 L0–L4；v1/v2/v3 只是切片），还是按用户旅程切？ |
...
模板要点：**评测集建设与人工抽检是显式任务**，不是顺便做。把它们写进计划表，才不会在排期紧张时被砍掉。

## CC/CD：接到迭代节奏上

把「两快一慢」接到 [AI 产品开发生命周期（CC/CD）](ai-lifecycle.md)：先做 **CD 0** 可行性与 spike，再按 **L0–L4** 切版本进入 CD 1–3，上线后走 CC 4–6。阶段表、准入信号和错误模式模板以生命周期页为准，本页不另写一套摘要。

经营阶段（导入 / 成长 / 成熟 / 衰退）见 [产品生命周期](product-lifecycle.md)，不要和 CC/CD 混成一个框架。
...
每个高风险区域（模型效果、数据、成本、合规、发布）指定一个 **DRI（Directly Responsible Individual，直接负责人）**：出事找这个人，不是大家负责。
...
-   **按代理权切**（推荐）：对外可用 v1 / v2 / v3 命名，对内必须落到 [CC/CD 生命周期](ai-lifecycle.md) 的 L0–L4。最小切片常见是 L0 建议 → L2 确认后执行 → L3 受限自治，不要跳过 L0 直接自动执行
-   **按用户旅程切**：先做提问 → 回答闭环，再做回答 → 执行
-   **按风险切**：先做低风险动作，不可逆动作最后做

### Must / Should / Could

范围分类用 MoSCoW，步骤与误用见[思维模型 · MoSCoW](../intro/thinking-models/moscow.md)；需求排序仍回[需求分析](requirements.md)的 RICE / WSJF。下表是交付迭代里的用法：
...
AI 产品的依赖比传统软件更多且更难控制：模型 API（能力、限流、价格随时可能变）、数据源（权限、时效、质量）、算力（训练 / 评测跑批的排队）、法务审批（数据合规的周期不可压缩）。规则：**外部依赖全部提前两个迭代排期**，并在计划模板里显式列一行依赖。模型升级、数据接入这类依赖一旦卡住，整个迭代都会停摆，这是 AI 项目最常见的隐性延期源。
...
跨部门推动靠证据和共同业务结果，不靠职级。评测集通过线、错误分派率、毛利护栏比「我觉得该做」更容易让工程、法务和销售签字；冲突写进需求的非目标与风险节，让四方在评审会上对齐。影响力的产物是一份带证据的决策记录，不是一次说赢了的会议。
...
-   **为 AI 而 AI**：堆 AI 功能却没有业务结果。立项时先写价值假设，用 OKR 的业务结果口径验收
-   **把评测当一次性工程**：评测集要随线上行为持续扩充，两个月不更新就失真

## 发布管理

发布管理的目标是：**把模型输出不可控变成发布过程可控**。对传统软件，发布是终点；对 AI 产品，发布只是进入校准循环的过渡（见 [CC/CD 生命周期](ai-lifecycle.md)）。发布是放出去并盯住。

### 评测作为发布门禁

把评测作为发布门禁写成可执行流程，而不是一句原则：
