---
description: 需求分析方法：从机会识别、可证伪假设、证据层级到优先级与拆解，给出 P0-P3 治理和 AI 特判五查，把模糊诉求变成可验收的需求。
---
...
1.  **收集**：把各种来源的模糊诉求原样收进来，先不评判（详见下文「需求来源」与「机会识别与产品策略输入」）
...
本页在四步之上扩展为六环节。四步是作业顺序，六环节是同一条链上的决策点，对应关系如下。

| 六环节 | 对应四步 | 关键产出 | 详见 |
| --- | --- | --- | --- |
| 机会识别 | 收集（源头） | 机会候选空间 | 「机会识别与产品策略输入」 |
| 假设 | 收集（加工） | 可证伪的假设 | 「JTBD 与 Y 模型」 |
| 证据 | 判断（依据） | 证据与置信度 | 「证据强度与样本思维」 |
...
所有模型都是沟通工具：把判断依据显性化，让团队吵得起来、吵得完。**同一个需求在不同模型下结论不同是常态**，差异本身就是讨论材料。提问结构（MoSCoW、第一性原理、PRC、5W2H1R、GROW）见[思维模型](../intro/thinking-models/index.md)；下面五个模型回答的是需求排序，不要和思维模型页互相替代。
...
**1. 能力可行性 spike**：先用 Playground/官方 API 拿 10-50 条真实样本试跑，再写完整需求。spike 要记录：能稳定完成什么、在什么输入上失败、输出格式是否可控。提示词实验方法参考 [Anthropic 官方提示词工程指南](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/overview)（或对应厂商文档）。spike 结果直接写进需求的"能力边界"节：**模型做不到的事，需求里要写"拒绝回答"而不是"尽力而为"**。
