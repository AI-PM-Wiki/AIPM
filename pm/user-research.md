---
description: 用户研究方法：按发现、验证、监控三种时机选择访谈、问卷、可用性测试与行为数据，降低决策不确定性，并把洞察写回需求与 PRD。
---
...
**研究频率**：好团队把研究做成连续动作。Cagan 的定性测试节奏是**每周 2～3 场**（每场约 1 小时、通常 **1 名用户**，见[《启示录》笔记](../case/books/inspired.md)）；Torres 的持续发现也是小步、高频、与交付并行。不要把「每周 2～3 场」读成「每周 6～15 人」——那会和下面的 5～8 人饱和样本打架。上线前集中做三个月研究、然后半年不做的团队，决策质量会随信息老化一起下降。
...
竞品分析是本栏目的竞品方法专节：产出是差异化机会与避坑清单，直接进入[需求分析](requirements.md)，不另开独立页。
...
-   **B 端角色链路**：buyer / user / sponsor / operator 都要覆盖，只访谈用户会漏掉买单逻辑（详见[需求分析](requirements.md#多角色与产品类型)）
-   **避免只访问友好客户**：推荐来的客户说话客气、问题不痛；从工单、流失名单、客服记录里招募，样本才有代表性
-   **避免"样本即结论"**：8 个用户说"很想要"，只说明问题存在，不说明市场规模——样本量对应证据层级（见[需求分析](requirements.md#证据强度与样本思维)）
...
可用性测试回答"用户能不能用、卡在哪"，诊断式研究更进一步回答"为什么卡住"。本页写**何时测、测什么、发现如何进需求**；样本量公式、SUS / HEART、A/B 实验与度量体系见[设计评审与可用性度量](design-evaluation.md)，不要两页各建一套指标。
...
-   **跨团队分发**：研究的终点是决策。洞察要进需求文档与 PRD（「来自 6 次访谈 + 500 张工单日志」作为证据标注，写法见[需求分析](requirements.md#证据强度与样本思维)），设计团队要拿到原始片段（引语、录像），工程团队要拿到行为数据结论
