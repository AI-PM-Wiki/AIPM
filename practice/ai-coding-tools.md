...
```mermaid
flowchart LR
    completion[单行 / 多行补全] --> chat[对话式生成]
    chat --> multi[多文件修改]
    multi --> e2e[端到端编码 Agent]
    completion -.低风险：删除即回退.-> trust[信任与验证]
    chat -.手动粘贴与核对.-> trust
    multi -.diff 审查与确认.-> trust
    e2e -.沙箱、测试、回滚、预算.-> trust
```

这条阶梯显示能力越高、执行范围越大，越需要把上下文、验证与回滚机制一起产品化，而不是只升级模型。
