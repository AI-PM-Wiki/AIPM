...
```mermaid
flowchart TD
    idea[产品想法] --> scope[按市场与业务识别数据、内容、行业红线]
    scope --> design[需求与技术方案评审]
    design --> controls[隐私、审核、标识、备案与版权控制]
    controls --> gate{上线验收通过？}
    gate -->|否| fix[整改、补材料或调整产品范围]
    fix --> design
    gate -->|是| launch[灰度上线]
    launch --> monitor[举报、审计、指标与监管巡检]
    monitor -->|重大变更| design
```

这条合规链说明合规不是发布前的一次签字，而是从市场识别开始、随产品变更持续回到评审的控制回路。
