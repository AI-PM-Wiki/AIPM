---
---
description: Copilot 形态产品设计：嵌入既有工作流、上下文感知、建议及时性与用户信任。
---
...
截断策略：按相关度排序后取前 N 段，不是「从末尾截」。无关内容宁可不要，也不要挤掉高优先级片段。
...
-   **企业部署**：管理员开关（按部门/角色启用功能）、策略控制（哪些能力可关、数据是否可出境）、审计日志。B 端客户关心的是"能关掉什么"，不是功能列表
