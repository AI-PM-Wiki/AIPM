...
| **负载均衡** | 在同一服务的多个实例间分配请求 | 解决容量和单实例故障；通常不做业务鉴权或协议转换 |
...
服务之间还可以用 **gRPC**：基于 HTTP/2 的 RPC，契约用 Protobuf，适合内网服务调用。浏览器、移动端和外部合作方仍以 HTTP JSON 为主，常在网关做协议转译。产品评审里要问清：对用户暴露的是哪一种，超时、错误码和版本如何翻译。
...
- **负载均衡**：把请求分配给同一服务的多个实例，配合健康检查和连接排空；它不管鉴权、限流策略和业务路由，那些通常在网关。
...
| 上网关统一处理 | 网关的鉴权、限流、路由和高可用边界是否明确？业务规则有没有被塞进去？和负载均衡分别做什么？ |
...
- [身份认证与权限](identity-access.md)：登录、会话、授权和最小权限
- [数据库与数据存储术语](databases.md)：服务端的数据模型、事务与存储选择
- [消息队列与异步系统](message-queues.md)：异步任务、重试和消费可靠性
- [部署与环境](deployment.md)：无状态、扩容和发布时的连接排空

### 来源说明

本文为后端工程通识整理，具体协议和实现以官方文档为准（访问日期 2026-09-04）：
