...
- `check_chart_flow.py` —— 图表语境。15 条:位图送的是图像本身且模型确实收到了、
  取图的三道限制(来源 / 类型 / 体积)、Mermaid 取源时序、SVG 取不到时的回落、
  不可信 SVG 不执行、「仅本机」那条边界、配额降级之后重新生成、模型不收图时的反馈。
  其中几条用的是「读不完的响应」:同一个地址先是页面自己那次加载、再是取源那次请求,
  服务端按 `Sec-Fetch-Dest` 分给两份内容,并记下取源实际读走多少、有没有被取消。
  这个类把 **Service Worker 关掉**——站点那个按地址做 cache-first,会把第二次请求
  挡回第一次的响应,两份内容根本到不了服务端(见 `harness.py` 的 `_Stream`)。
