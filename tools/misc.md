...
AI 产品经理日常使用的效率工具集。工具更新快，本页保持「类别 + 代表性工具 + 选型一句话」的目录式维护：每类只列经过验证的代表作，不追求收录全。本页只维护效率工具清单，不做模型对比：模型能力与定价对比见 [模型能力与边界](../ai/capabilities.md)，成本测算方法见 [LLM 成本测算](llm-cost.md)。
