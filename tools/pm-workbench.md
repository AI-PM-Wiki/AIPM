---
---
description: PM 团队协作与管理工具全景：知识库、项目任务、需求缺陷、埋点分析与用户研究。
---
...
???+ note "范围界定"
    - 本页覆盖协作半径内的工具：多人共享、流程驱动、权限与数据集中
...
```mermaid
flowchart LR
    docs[文档与知识库] --> tasks[项目与任务]
    tasks --> issues[需求与缺陷]
    issues --> analytics[埋点与数据分析]
    analytics --> research[用户研究]
    research --> im[协作沟通]
    im --> docs
```

工作台把方案、执行、反馈和协作连成循环；每个环节选主工具，再用集成与导出保持上下文可追溯。
...
个人库与团队库之间用定期整理、手动发布衔接：个人笔记直接搬进团队库会造成噪音，发布前要经过编辑与脱敏。

### AI 加持

- **知识库问答**：基于 RAG 的问文档：新同事问「灰度流程是什么」直接得到带来源的答案，检索与生成方法见 [评估与评测](../ai/evaluation.md)
- **文档生成**：会议纪要转文档、周报聚合（从任务与文档动态汇总）
- 知识库问答的效果取决于文档质量与权限隔离：公开文档和机密文档混在一个索引里是事故
...
「Kanban 做发现、Scrum 做交付」的混搭（[项目管理](../pm/project-management.md)）在工具上这样落地：**发现轨道**的验证任务（访谈、假门测试、可行性原型）用轻看板（Trello / Linear）按价值拉取，**交付轨道**的承诺任务进正式迭代（Jira / 禅道 / 飞书项目）。两个轨道分开跑，别把发现任务和交付任务混在一个看板里自嗨：合流点仍是每个迭代开始时把已验证的想法排进交付 backlog。
...
```mermaid
flowchart TD
    inputs[用户、客服、竞品与内部诉求] --> backlog[需求池：来源与证据分级]
    backlog --> sprint[迭代排期与任务拆解]
    sprint --> acceptance[按验收标准验证]
    acceptance -->|通过| release[发布与埋点观察]
    acceptance -->|不通过| bug[缺陷回流]
    release --> feedback[用户反馈与数据]
    feedback --> backlog
    bug --> sprint
```

需求和缺陷工具的价值在于可追踪闭环：证据进入需求池，验收结果和线上反馈再反哺下一轮排期。
...
缺陷自动分派（与客服工单路由同思路）、描述自动补全（日志 / 截图 / 上下文）、需求自动去重与归类、优先级建议。**建议只做输入，裁决仍是人的事**，呼应 [需求分析](../pm/requirements.md) 的「评分表是沟通工具，不是结论」。
...
```mermaid
flowchart LR
    event[事件 + 属性 + 用户标识] --> warehouse[数仓清洗与建模]
    warehouse --> dashboard[BI 看板监控]
    dashboard --> attribution[实验与归因分析]
    attribution --> decision[产品决策]
    decision --> event
```

数据分析工具链以统一事件模型为入口，以实验归因为决策出口，闭环结果再指导下一轮埋点。
...
NL2SQL（自然语言查数）、自动洞察（异常检测、波动归因）、看板自动解读。NL2SQL 的正确用法是生成 SQL 后人工 review：口径错了 AI 不会知道。能力以官方发布为准。
...
**证据层级对应**（详见 [需求分析](../pm/requirements.md#证据强度与样本思维)）：访谈 = 定性证据，问卷 = 描述性数据，埋点行为数据 = 行为证据，业务结果与实验 = 最高层证据。工具链要覆盖多层，只用问卷说明不了用户实际怎么做。
...
???+ warning "先看数据出口"
    换工具的迁移成本是选型时最容易被低估的成本：文档、任务、历史数据一旦锁死，换工具的代价是重录一遍。
